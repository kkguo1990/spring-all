...
public class RedisSyncAspect {
	...
	@Around("@annotation(com.kk.redis.lock.Synchronized) execution(* *.*(..))")
    public Object around(ProceedingJoinPoint pjp) throws Throwable {
        ...
        LOGGER.debug("获取分布式锁，namespace=[{}], expireSecond=[{}], waitLockInMills=[{}], threadName=[{}], targetMethod=[{}]", namespace, expireSecond, waitLockMillisecond, threadName, targetMethod.getName());

		int a = 0, b = 1; // 构造兔子数列，最大1000
		
		long start = System.currentTimeMillis();
		while(true) {
			boolean requireLock = redisLockHelper.requireLock(namespace, "-", expireSecond);
			if(requireLock) {
				try {
